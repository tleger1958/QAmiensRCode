...
public:
	 ...
	 * The ECC level of the result may be higher than the ecl argument if it can be done without increasing the version.
	 */
	static QAmiensRCode encodeBinary(const std::vector<uint8_t> &data, const Ecc &ecl);
...
public:
	 ...
	 */
	QAmiensRCode(int ver, const Ecc &ecl, const std::vector<uint8_t> &dataCodewords, int mask);
	 ...
	 */
	QAmiensRCode(const QAmiensRCode &qr, int mask);
